...
opts = Trollop::options do
  ...
  opt :flood_name, "Unique name of the flood to be ran <only applicable to prod mode>", :type => :string, default: "Squarefoot SRP Test #{Time.now}"
...
#end.run(gui: true)
end.flood(opts[:api_key], {privacy: 'public', files: ["#{Dir.pwd}/srp_query_decoded.csv"], region: opts[:region], name: opts[:flood_name], grid: opts[:api_grid]})